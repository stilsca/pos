...
<div class="modal fade" id="modalEliminar" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <form action="<?php echo base_url(); ?>/Ventas/anular" method="post">
...
<script>
    function listar() {
        ...
        if (desde.length > 2 & hasta.length > 2) {
            ...
            $.ajax({
                url: '<?= base_url(); ?>/Ventas/listadoJSON',
                method: 'post',
                data: {
                    desde,
                    hasta
                ...
                success: (res) => {
                    for (var i = 0; i < res.ventas.length; i++) {
                        var btn = '<a target="_blank" href="<?= base_url(); ?>/Ventas/comprobante/' + res.ventas[i]['idVenta'] + '" class="btn btn-info" title="Ver comprobante"><i class="fa fa-search"></i></a><a href="#" class="btn btn-danger" title="Anular" data-toggle="modal" data-target="#modalEliminar" data-id="' + res.ventas[i]['idVenta'] + '"><i class="fa fa-trash"></i></a>';
                        var insertar = '<tr><td>' + res.ventas[i]['fech'] + '</td><td>' + res.ventas[i]['regimen'] + '-' + res.ventas[i]['nroComprobante'] + '</td><td>' + res.ventas[i]['razonSocial'] + '</td><td class="text-right">' + new Intl.NumberFormat("de-DE").format(res.ventas[i]['total']) + '</td><td>' + btn + '</td></tr>';
